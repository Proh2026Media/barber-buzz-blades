...
alter table public.customer_surveys add constraint customer_surveys_question_check check(question in ('discovery','period','professional','frequency','conversation','service_interest','satisfaction','improvement'));
...
begin
  ...
  if q is null and not exists(select 1 from customer_surveys where user_id=auth.uid() and question='improvement' and shown_at>now()-interval '180 days') then
    select s.appointment_id,s.appointment_starts_at into target_id,target_start from customer_surveys s join appointments a on a.id=s.appointment_id
    where s.user_id=auth.uid() and a.customer_id=auth.uid() and a.status='completed' and a.ends_at<=now() and s.question='satisfaction' and s.state='answered'
      and s.answer in('1','2','3','4','5') and s.shown_at>now()-interval '180 days' and not exists(select 1 from customer_surveys other where other.user_id=auth.uid() and other.question='improvement' and other.appointment_id=s.appointment_id)
  ...
  if q is null and not exists(select 1 from customer_surveys where user_id=auth.uid() and question='satisfaction' and shown_at>now()-interval '180 days') then
    select a.id,a.starts_at into target_id,target_start from appointments a where a.customer_id=auth.uid() and a.status='completed' and a.ends_at<=now() and a.ends_at>=now()-interval '90 days'
      and not exists(select 1 from customer_surveys s where s.user_id=auth.uid() and s.question='satisfaction' and s.appointment_id=a.id) order by a.ends_at desc,a.id limit 1;
    if target_id is not null then q:='satisfaction'; end if;
  end if;
  if q is null then
    select candidate into q from unnest(array['period','professional','frequency','conversation','service_interest']) with ordinality as choices(candidate,position)
...
begin
  ...
  update customer_surveys set answer=p_answer,state=case when p_answer is null then 'dismissed' else 'answered' end,answered_at=now() where id=p_id;
end $$;
